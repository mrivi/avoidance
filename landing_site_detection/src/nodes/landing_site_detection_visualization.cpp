...
void LandingSiteDetectionVisualization::publishMean(const Grid &grid) {
  ...
  for (size_t i = 0; i < std::ceil(grid.grid_size_ / grid.cell_size_); i++) {
    for (size_t j = 0; j < std::ceil(grid.grid_size_ / grid.cell_size_); j++) {
      ...
      cell.pose.position.x = (i * grid.cell_size_) + grid_min.x() + (grid.cell_size_ / 2.f);
      cell.pose.position.y = (j * grid.cell_size_) + grid_min.y() + (grid.cell_size_ / 2.f);
      cell.pose.position.z = 0.0;

      float h = ((range_max - range_min) * (mean(i, j) - variance_min_value) / (variance_max_value - variance_min_value) ) +  range_min;
...
void LandingSiteDetectionVisualization::publishStandardDeviation(const Grid &grid){
  ...
  for (size_t i = 0; i < std::ceil(grid.grid_size_ / grid.cell_size_); i++) {
    for (size_t j = 0; j < std::ceil(grid.grid_size_ / grid.cell_size_); j++) {
      ...
      float h = ((range_max - range_min) * (sqrtf(variance(i, j)) - variance_min_value) / (variance_max_value - variance_min_value) ) +  range_min;
      float red, green, blue;
      float max_aa = 1.f;
      HSVtoRGB(red, green, blue, h, max_aa, max_aa);
      cell.color.r = red;
...
void LandingSiteDetectionVisualization::publishGrid(const Grid &grid, const geometry_msgs::Point& pos, float smoothing_size) const{
  ...
  for (size_t i = 0; i < std::ceil(grid.grid_size_ / grid.cell_size_); i++) {
    for (size_t j = 0; j < std::ceil(grid.grid_size_ / grid.cell_size_); j++) {
      ...
      if (i>=(offset - smoothing_size) && i < (offset + smoothing_size) && j>= (offset - smoothing_size) && j < (offset + smoothing_size)) {
        cell.scale.z = 0.8;
      }
      marker_array.markers.push_back(cell);
      cell.id += 1;
