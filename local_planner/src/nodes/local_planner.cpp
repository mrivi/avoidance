...
void LocalPlanner::determineStrategy() {
  ...
  if (!reach_altitude_) {
    printf("goalz %f takeoff z %f \n", goal_.z(), take_off_pose_.pose.position.z );
    starting_height_ =
        std::max(goal_.z() * 0.5, take_off_pose_.pose.position.z + 1.0);
    printf("\033[1;35m[OA] Reach height (%f) first: Go fast\n \033[0m",
             starting_height_);
    waypoint_type_ = reachHeight;
    printf("z %f goal_acceptance %f \n", pose_.pose.position.z, starting_height_);
