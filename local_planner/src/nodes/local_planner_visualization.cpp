...
void LocalPlannerVisualization::publishTree(const std::vector<TreeNode>& tree, const std::vector<int>& closed_set,
  ...
  float range_min = 180.f;

  std::cout << tree.size() << std::endl;
  for (size_t i = 0; i < tree.size(); i++) {
    variance_max_value = std::max(variance_max_value, tree[i].total_cost_);
    variance_min_value = std::min(variance_min_value, tree[i].total_cost_);
  }
  for (size_t i = 0; i < tree.size(); i++) {
    int node_nr = closed_set[i];
    marker.pose.position = toPoint(tree[i].getPosition());
    float cost = tree[i].total_cost_;
    float heuristic = tree[i].heuristic_;
    std::cout << tree[i].getPosition() << " " << cost <<std::endl;
