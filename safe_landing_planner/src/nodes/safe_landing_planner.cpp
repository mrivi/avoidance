    ...
    if (!std::isnan(xyz.x) && !std::isnan(xyz.y) && !std::isnan(xyz.z)) {
      ...
      if (isInsideGrid(xyz.x, xyz.y)) {
        ...
        std::pair<float, float> mean_variance = computeOnlineMeanVariance(
            prev_mean, prev_variance, xyz.z,
            static_cast<float>(grid_.getCounter(grid_index)));
...
void SafeLandingPlanner::isLandingPossible() {
  ...
  if (n_lines_padding_ > 0) {
    Eigen::MatrixXi land_padded(size + 2 * n_lines_padding_,
                                size + 2 * n_lines_padding_);
    Eigen::MatrixXi land_accumulator(size + 2 * n_lines_padding_,
                                     size + 2 * n_lines_padding_);
    land_padded.fill(0);
    ...
    for (int i = n_lines_padding_; i < land_padded.rows() - n_lines_padding_;
         i++) {
      for (int j = n_lines_padding_; j < land_padded.cols() - n_lines_padding_;
           j++) {
    ...
    // neighborhood
    land_accumulator = (land_accumulator.array() <= min_n_land_cells_)
                           .select(0, land_accumulator);
    land_accumulator = (land_accumulator.array() > min_n_land_cells_)
                           .select(1, land_accumulator);
    ...
    grid_.land_.block(0, 0, grid_.land_.rows(), grid_.land_.cols()) =
        land_accumulator.block(n_lines_padding_, n_lines_padding_,
...
bool SafeLandingPlanner::isInsideGrid(float x, float y) {
  ...
  return x < grid_max.x() && x > grid_min.x() && y < grid_max.y() &&
         y > grid_min.y();
}
...
std::pair<float, float> SafeLandingPlanner::computeOnlineMeanVariance(
    float prev_mean, float prev_variance, float new_value, float seq) {
...
void SafeLandingPlanner::dynamicReconfigureSetParams(
